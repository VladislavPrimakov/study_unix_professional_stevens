...
#define print_compile(name) \
		std::cout << std::setw(SIZE_NAME) << std::left << #name << std::setw(SIZE_VALUE) << #name << std::endl;

#define print_not_defined(name) \
		std::cout << std::setw(SIZE_NAME) << std::left << #name << std::setw(SIZE_VALUE) << "(not defined)"<< std::endl;
...
int main(int argc, char* argv[]) {
	...
	try {
		std::cout << std::setw(SIZE_NAME) << std::left << "Limit Name" << std::setw(SIZE_VALUE) << "Value" << std::endl;
#ifdef ARG_MAX
...
void pr_sysconf(const std::string& mesg, int name) {
	long val;
	std::cout << std::setw(SIZE_NAME) << std::left << mesg << std::setw(SIZE_VALUE);
	errno = 0;
	...
	if ((val = pathconf(path.c_str(), name)) < 0) {
		if (errno != 0) {
			if (errno == EINVAL)
				...
				throw UnixError("pathconf error for path " + path);
		} else {
			std::cout << "(no limit)" << std::endl;
		}
	} else {
